        ...
        <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl font-display font-bold text-white">Lịch sử giao dịch</h2>
                </div>
                {activeTab === 'orders' && (
                    <button
            ...
            <div className="flex items-center gap-2 border-b border-white/10 pb-4">
                <button
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${activeTab === 'deposits' ? 'bg-white/10 text-white border border-white/20' : 'text-gray-400 hover:text-white hover:bg-white/5 border border-transparent'}`}
                >
                    <i className="fa-solid fa-money-bill-transfer mr-2"></i> Giao dịch nạp tiền ({initialTransactions.length})
                </button>
                <button
                    onClick={() => setActiveTab('orders')}
                    className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors ${activeTab === 'orders' ? 'bg-white/10 text-white border border-white/20' : 'text-gray-400 hover:text-white hover:bg-white/5 border border-transparent'}`}
                >
            ...
            {activeTab === 'deposits' ? (
                ...
                (!initialTransactions || initialTransactions.length === 0) ? (
                    <div style={{ background: 'rgba(255, 255, 255, 0.06)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' }} className="border border-white/10 shadow-2xl rounded-2xl p-6 min-h-[400px] flex flex-col items-center justify-center text-center group hover:border-brand-accent/30 transition-colors">
                        ...
                        <h3 className="text-lg font-bold text-white mb-2">Chưa có giao dịch nạp tiền</h3>
                        <p className="text-gray-500 font-medium max-w-sm">Dữ liệu nạp tiền của bạn sẽ xuất hiện tại đây một cách an toàn.</p>
                    </div>
                ) : (
                    <div style={{ background: 'rgba(255, 255, 255, 0.03)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }} className="border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="border-b border-white/5 bg-white/5">
                                        <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Thời gian</th>
                                        <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Loại</th>
                                        <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Số tiền</th>
                                        <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Trạng thái</th>
                                        <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Mã tham chiếu</th>
                                    </tr>
                                </thead>
                                    ...
                                    {initialTransactions.map((tx) => (
                                        <tr key={tx.id} className="hover:bg-white/[0.02] transition-colors group">
                                            <td className="px-6 py-4">
                                                <div className="text-sm font-medium text-white">{formatDate(tx.created_at)}</div>
                                                <div className="text-[10px] text-gray-500 mt-0.5">ID: {String(tx.id).slice(0, 8)}...</div>
                                            </td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="text-sm text-gray-400 font-medium truncate max-w-[200px]" title={tx.content || tx.gateway_id}>
                ...
                (!orders || orders.length === 0) ? (
                    <div style={{ background: 'rgba(255, 255, 255, 0.06)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' }} className="border border-white/10 shadow-2xl rounded-2xl p-6 min-h-[400px] flex flex-col items-center justify-center text-center group hover:border-brand-accent/30 transition-colors">
                        ...
                        <p className="text-gray-500 font-medium max-w-sm">Dữ liệu đơn hàng dịch vụ của bạn sẽ xuất hiện tại đây.</p>
                    </div>
                ) : (
                    <div style={{ background: 'rgba(255, 255, 255, 0.03)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }} className="border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="border-b border-white/5 bg-white/5">
                                        <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Thời gian</th>
                                        <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Link / Dịch vụ</th>
                                        <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Số lượng</th>
                                        <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Chi phí</th>
                                        <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Trạng thái</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-white/5">
                                    {orders.map((order) => (
                                        <tr key={order.id} className="hover:bg-white/[0.02] transition-colors group">
                                            <td className="px-6 py-4">
                                                <div className="text-sm font-medium text-white">{formatDate(order.created_at)}</div>
                                                <div className="text-[10px] text-gray-500 mt-0.5">ID: {String(order.id).slice(0, 8)}...</div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="text-sm font-medium truncate max-w-[250px]" title={order.services?.name ? `[${order.service_id}] ${order.services.name}` : `Dịch vụ ID: ${order.service_id}`}>
                                                    {order.services?.name ? <span className="text-brand-accent">[{order.service_id}] {order.services.name}</span> : <span className="text-gray-300">Dịch vụ ID: {order.service_id}</span>}
                                                </div>
                                                <div className="text-[11px] text-gray-400 font-medium mt-1 truncate max-w-[250px]" title={order.target_link}>
